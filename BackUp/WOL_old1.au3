		...
		For $i = 0 To UBound ( $starind ) - 1 Step 1
			...
			If @error Then
				...
				Else
					;MsgBox ( 1, "", $val )
					If $isip = False Then
						Run ( @ComSpec & ' /c paexec.exe \\' & IniRead ( "C:\WOL\devices.ini", StringTrimLeft ( StringTrimRight ( $info[$starind[$i]], 1 ), 1 ), "Name", "" ) & ' -h powercfg /deviceenablewake "' & $val & '"', "C:\WOL", @SW_HIDE, $STDERR_MERGED )
					Else
						Run ( @ComSpec & ' /c paexec.exe \\' & IniRead ( "C:\WOL\devices.ini", StringTrimLeft ( StringTrimRight ( $info[$starind[$i]], 1 ), 1 ), "IPAddress", "" ) & ' -h powercfg /deviceenablewake "' & $val & '"', "C:\WOL", @SW_HIDE, $STDERR_MERGED )
					EndIf
					ProcessWaitClose ( $proc )
					If Not StringIsSpace ( StderrRead ( $proc, True ) ) Then
						MsgBox ( 1, "", StderrRead ( $proc ) )
					Else
						MsgBox ( 1, "", StdoutRead ( $proc ) )
					EndIf
				EndIf
			Else
				ContinueLoop
